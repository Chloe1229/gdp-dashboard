...
if st.session_state.step == 7:
    ...
    if "step7_rows" not in st.session_state:
        ...
        def _load_rows():
            zf = zipfile.ZipFile("step7_data.xlsx")
            with zf.open("xl/sharedStrings.xml") as f:
                shared = [t.text if t.text is not None else "" for t in ET.parse(f).findall('.//{http://schemas.openxmlformats.org/spreadsheetml/2006/main}t')]
            with zf.open("xl/worksheets/sheet1.xml") as f:
                root = ET.parse(f).getroot()
            ns = '{http://schemas.openxmlformats.org/spreadsheetml/2006/main}'
            rows_xml = root.findall(f'.//{ns}row')
            headers = [shared[int(c.find(f"{ns}v").text)] if c.get('t')=='s' else c.find(f"{ns}v").text for c in rows_xml[0].findall(f'{ns}c')]
            records = []
            for row in rows_xml[1:]:
                cells = [shared[int(c.find(f"{ns}v").text)] if c.get('t')=='s' else c.find(f"{ns}v").text if c.find(f"{ns}v") is not None else '' for c in row.findall(f'{ns}c')]
                rec = {h: (cells[i] if i < len(cells) else '') for i, h in enumerate(headers)}
                records.append(rec)
            return records
        st.session_state.step7_rows = _load_rows()
    ...
    for row in st.session_state.step7_rows:
        if row.get("title_key") != current_key:
            continue
        cond_parts = []
        for field in ("subitem_met", "requirements_met", "requirements_unmet"):
            expr = row.get(field, "").replace('""', '"').strip()
            if not expr:
                continue
    ...
    if hits:
        for row in hits:
            ...
            st.session_state.step7_results.setdefault(current_key, []).append(
                (row.get("output_1_tag", ""), row.get("output_1_text", ""), row.get("output_2_text", ""))
            )
    else:
        st.warning(
